...
<body>
	...
	<div class="body">
			...
			<div>
				<p class="tieude">Chỉnh sửa thông tin của xe có mã: <%=session.getAttribute("maxe") %></p>	
			</div>
			<div style="margin-left: 7%; margin-right: 7%; width: 84%;margin-top: 0px;display: flex;">
				<div class="leftbody">
					<p class="nd">Thông tin hiện tại</p>
					<div>
						<p class="nd2">Mã xe: <%=session.getAttribute("maxe") %></p>
		                <p class="nd2">Tên xe: <%=session.getAttribute("tenxe") %></p>
		                <p class="nd2">Số lượng: <%=session.getAttribute("soluongtk") %></p>
		                <p class="nd2">Giá cơ bản: <%=session.getAttribute("gia") %></p>
		                <p class="nd2">Tên hãng xe: <%=session.getAttribute("tenhangxe") %></p>
		                <p class="nd2">Ngày nhập: <%=session.getAttribute("ngaynhap") %></p>
		                <img style="width: 250px; height: 150px;margin-left: 54px;" src="<%=session.getAttribute("anh")%>">
					</div>
				</div>
				<div class="rightbody">
					<p class="nd">Thông tin mới</p>
					<div style="width: 350px;text-align: center;display: flex;justify-content: center;margin-top: -20px;">
						<form style="width: 250px;" action="suaxe?addnn=<%=java.time.LocalDate.now()%>" method="post">
							<input style="visibility: hidden;width: 0px;height: 0px;" required="required" name="addmaxe" placeholder="Nhập mã xe" type="text" value="<%=session.getAttribute("maxe") %>">
							<input class="nd3" required="required" name="addtenxe" placeholder="Nhập tên xe" type="text">
							<input class="nd3" required="required" name="addsoluong" placeholder="Nhập số lượng" type="text">
							<input class="nd3" required="required" name="addgia" placeholder="Nhập giá" type="text">
							<select name="addmahangxe" style="width: 250px;height: 30px;border-radius: 5px;">
		                   	    	<%
